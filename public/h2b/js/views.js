...
H2B.views = (function () {
  ...
  async function renderNews() {
    ...
    try {
      ...
      const html = [];
      html.push(`<div class="news-card"><h3>📊 Situação do feed</h3><div class="news-date">${t.lastFeed ? 'última publicação vista: ' + H2B.fmtDate(t.lastFeed) : 'sem publicação registrada'}</div><p>${t.total || 0} vagas abertas · ${t.h2a || 0} H-2A · ${t.h2b || 0} H-2B · ${t.withEmail || 0} com e-mail de candidatura.</p></div>`);
      runs.forEach(r => {
        const m = safeJson(r.metrics_json || r.result_json, null) || {};
        const errs = safeJson(r.errors_json, []) || [];
        html.push(`<div class="news-card"><h3>${errs.length ? '⚠️ Importação com erro' : '📥 Importação do DOL'}</h3><div class="news-date">${fmtDateTime(r.created_at || r.started_at)} · ${r.duration_ms ? Math.round(r.duration_ms / 1000) + 's' : ''}</div><p>${errs.length ? esc(errs.join('; ')) : `${m.total !== undefined ? m.total + ' no feed' : ''}${m.inserted !== undefined ? ' · ' + m.inserted + ' novas' : ''}${m.updated !== undefined ? ' · ' + m.updated + ' atualizadas' : ''}${m.filtered !== undefined ? ' · ' + m.filtered + ' filtradas' : ''}` || 'Concluída.'}</p></div>`);
      });
  ...
  async function renderSettings() {
    ...
    $('#st-import').onclick = async () => { const b = $('#st-import'); b.disabled = true; b.innerHTML = '<span class="spin spin-sm"></span> Importando…'; try { const r = await API.seasonal.import({}); const m = r.metrics || {}; toast(`Importado: ${m.inserted !== undefined ? m.inserted + ' novas, ' : ''}${m.total !== undefined ? m.total + ' no feed' : 'ok'}`, 'ok'); await H2B.refreshCore(); renderSettings(); } catch (e) { err(e); b.disabled = false; b.innerHTML = '<i class="ti ti-download"></i> Importar agora'; } };
