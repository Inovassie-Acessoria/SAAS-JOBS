...
H2B.views = (function () {
  ...
  async function renderSettings() {
    ...
    el.innerHTML = `
      ...
      <div class="prof-card" style="margin-bottom:12px">
        ...
        <div class="prof-card-bd">
          ...
          <div class="hint">A importação lê o feed oficial (20 dias) <b>e</b> o índice do site (todas as vagas ativas), e atualiza o estado ativa/inativa de toda a base.</div>
          <div style="display:flex;gap:6px;flex-wrap:wrap"><button class="btn btn-primary btn-sm" id="st-save-feed"><i class="ti ti-device-floppy"></i> Salvar</button><button class="btn btn-secondary btn-sm" id="st-test-feed"><i class="ti ti-plug"></i> Testar</button><button class="btn btn-secondary btn-sm" id="st-import"><i class="ti ti-download"></i> Importar agora</button><button class="btn btn-secondary btn-sm" id="st-dol-sync"><i class="ti ti-refresh-dot"></i> Atualizar estado no DOL</button></div>
        </div>
      </div>
      ...
      <div class="prof-card" style="margin-bottom:12px">
        ...
        <div class="prof-card-bd">
          <div class="field"><label>Limite global por dia — teto <b>${q.absoluteCap || 300}</b> (${q.perAccountCap || 300} × ${senders.active || 1} conta(s) ativa(s))</label><input class="input" id="st-limit" type="number" min="0" max="${q.absoluteCap || 300}" value="${Number(cfg.daily_email_limit) || 0}" placeholder="0 = automático"><div class="hint">0 = automático: acompanha o teto (300 por conta Gmail cadastrada). Conectar mais contas aumenta o teto na hora.</div></div>
          <div class="field"><label>Intervalo por destinatário (dias)</label><input class="input" id="st-cooldown" type="number" min="0" max="365" value="${Number(cfg.recipient_cooldown_days) || 0}"><div class="hint">Um e-mail por caixa de entrada a cada N dias. Agentes aparecem em dezenas de vagas — sem isso o robô manda dezenas de e-mails para a mesma pessoa. 0 desliga (não recomendado).</div></div>
    ...
    $('#st-import').onclick = async () => { const b = $('#st-import'); b.disabled = true; b.innerHTML = '<span class="spin spin-sm"></span> Importando…'; try { const r = await API.seasonal.import({}); const m = r.metrics || {}; const q = (m.autoQueued && m.autoQueued.queued) || 0; const ds = m.dolStatus && !m.dolStatus.error ? ` · DOL: ${m.dolStatus.active} ativas, ${m.dolStatus.inactive} inativas` : ''; toast(`Importação: ${m.received || 0} recebidas (feed + índice) · ${m.newJobs || 0} novas · ${m.analyzed || 0} analisadas · ${m.recommended || 0} recomendadas${q ? ' · ' + q + ' na fila' : ''}${ds}`, 'ok', 9000); if (m.warnings && m.warnings.length) toast(m.warnings[0], undefined, 8000); await H2B.refreshCore(); renderSettings(); } catch (e) { err(e); b.disabled = false; b.innerHTML = '<i class="ti ti-download"></i> Importar agora'; } };
    $('#st-save-limit').onclick = async () => { try { await API.seasonal.saveConfig({ daily_email_limit: Number($('#st-limit').value) || 0, recipient_cooldown_days: Number($('#st-cooldown').value) || 0 }); toast('Limites salvos', 'ok'); await H2B.refreshCore(); renderSettings(); } catch (e) { err(e); } };
    const sync = $('#st-dol-sync'); if (sync) sync.onclick = async () => { sync.disabled = true; sync.innerHTML = '<span class="spin spin-sm"></span> Verificando…'; try { const r = await API.seasonal.dolSync(); toast(`DOL: ${r.active} ativas · ${r.inactive} inativas · ${r.unpublished} ainda sem página (${r.checked} verificadas)`, 'ok', 8000); renderSettings(); } catch (e) { err(e); sync.disabled = false; sync.innerHTML = '<i class="ti ti-refresh-dot"></i> Atualizar estado no DOL'; } };
